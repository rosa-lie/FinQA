#!/usr/bin/env python3
"""Audit ShareGPT-style SFT data and export suspicious samples for manual review."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--min_short_answer_chars", default=130, type=int)
    parser.add_argument("--min_long_prompt_chars", default=1800, type=int)
    parser.add_argument("--low_ratio_threshold", default=0.08, type=float,
                        help="Flag when gpt_len / max(1,human_len) is below threshold.")
...
def main() -> None:
    ...
    for obj in rows:
        ...
        h = safe_get_turn(obj, 0).strip()
        g = safe_get_turn(obj, 1).strip()
        by_prompt[h].append((ln, g, rid))

    conflict_lines: Dict[int, Dict[str, Any]] = {}
    for prompt, items in by_prompt.items():
        uniq = sorted({g for _, g, _ in items})
        if len(uniq) > 1:
            ...
            for ln, _, _ in items:
                conflict_lines[ln] = {
                    ...
                    "conflict_record_ids": [r for _, _, r in items],
    ...
    for obj in rows:
        ...
        human_len = len(h)
        gpt_len = len(g)
        ratio = gpt_len / max(1, human_len)

        reasons: List[str] = []
        severity = "medium"

        if gpt_len < args.min_short_answer_chars:
            reasons.append("short_answer")
        if human_len > args.min_long_prompt_chars and gpt_len < args.min_short_answer_chars:
            reasons.append("long_prompt_short_answer")
            severity = "high"
        if ratio < args.low_ratio_threshold:
            reasons.append("very_low_answer_prompt_ratio")
        if PAT_EVIDENCE_PLACEHOLDER.search(g):
            reasons.append("evidence_placeholder_text_1")
            severity = "high"
        if PAT_CONST.search(g):
        ...
        rec: Dict[str, Any] = {
            ...
            "human_len": human_len,
            "gpt_len": gpt_len,
            "answer_prompt_ratio": round(ratio, 6),
            "human_head": short(h),
            "gpt_head": short(g),
        }
        if ln in conflict_lines:
            rec.update(conflict_lines[ln])
        m = PAT_PROGRAM.search(g)
        if m:
            rec["program_head"] = short(m.group(1), 120)
